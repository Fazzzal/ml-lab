...
def supervisor(
    ...
    logger.info(
        f"Supervisor iteration: {retry_count}/{settings.MAX_ITERATIONS}"
    )

    if retry_count > settings.MAX_ITERATIONS:
        logger.warning(
        ...
        return {
            ...
            "supervisor_reason": (
                ...
                f"({settings.MAX_ITERATIONS}) reached; "
    ...
    if decision == "feature_engineering":
        logger.info("Routing to feature engineering agent.")
    ...
    if decision == "continue":
        if experiment_type == "baseline_model":
            ...
            # If the critic re-proposes a baseline that has
            # already run, don't loop on it again — treat the
            # research as complete instead of bouncing forever.
            if model and experiment_id not in experiment_history:
                logger.info(f"Routing to experiment agent: {model}")
            ...
            logger.warning(
                "Critic proposed a baseline model that "
                "has already been run (or is invalid); "
            ...
            return {
                ...
                "current_task": (
                    "Duplicate/invalid experiment proposal; stopping"
                ),
            }

        logger.warning("Invalid baseline experiment proposal.")
